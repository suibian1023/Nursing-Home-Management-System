  ...
  <div class="page-container">
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑护理等级' : '新增护理等级'" width="500px" @close="resetForm">
      <el-form :model="form" :rules="rules" ref="formRef" label-width="100px">
        ...
        <el-form-item label="等级编码" prop="levelCode"><el-input v-model="form.levelCode" /></el-form-item>
        <el-form-item label="费用(元/月)" prop="price"><el-input-number v-model="form.price" :min="0" :precision="2" style="width:100%" /></el-form-item>
...
const form = reactive({ id: null, levelName: '', levelCode: '', price: null, remark: '' })
const rules = { levelName: [{ required: true, message: '请输入等级名称', trigger: 'blur' }] }

const loadData = async () => { loading.value = true; try { const res = await getNurseLevelList(); tableData.value = res.data || [] } finally { loading.value = false } }
...
const handleDelete = async (id) => { await ElMessageBox.confirm('确定删除吗？', '提示', { type: 'warning' }); await deleteNurseLevel(id); ElMessage.success('删除成功'); loadData() }
const handleSubmit = async () => { const valid = await formRef.value.validate().catch(() => false); if (!valid) return; if (isEdit.value) { await updateNurseLevel(form); ElMessage.success('修改成功') } else { await addNurseLevel(form); ElMessage.success('新增成功') }; dialogVisible.value = false; loadData() }
const resetForm = () => { Object.assign(form, { id: null, levelName: '', levelCode: '', price: null, remark: '' }); formRef.value?.resetFields() }
onMounted(loadData)
</script>
