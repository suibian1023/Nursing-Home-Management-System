...
import { getBackdownPage, addBackdown, updateBackdown, deleteBackdown, getCustomerList, getBedList, getCustomerById } from '@/api'
...
const openEdit = async (row) => {
  isEdit.value = true
  await loadCustomerData()
  // 退住后的老人已逻辑删除(is_deleted=1)，/customer/list 不返回，
  // 需单独拉取并补入 customerList，否则 dropdownList 兜底逻辑拿不到当前老人
  if (row.customerId && !customerList.value.find(c => c.id === row.customerId)) {
    try {
      const res = await getCustomerById(row.customerId)
      if (res?.data) {
        customerList.value.push(res.data)
      }
    } catch (e) {
      console.error('加载退住老人信息失败:', e)
    }
  }
  Object.assign(form, {
    id: row.id,
